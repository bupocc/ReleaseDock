...
- 版本草稿、稳定版与预发布版、Markdown 预览（引用、链接、列表和代码等）、最新稳定版标记。
- 安装包拖放上传、文件重命名、平台和架构设置、草稿附件移除、正式发布与下架。
...
正式发布的版本内容和安装包不可覆盖，以保证版本号和校验值一致；修订软件请创建新版本。安装包名称可单独修改，具体方式见下文。预发布版不能设置为最新稳定版。
...
在版本页面的已上传文件旁，或后台「文件管理」中点击「重命名」，填写包含扩展名的完整名称后保存。草稿、已发布及已下架的附件均可操作；名称修改后，列表、发布预览和浏览器下载文件名同步更新。

名称最多 180 个字符，支持中文。同一版本不能存在同名文件，名称不能包含路径、控制字符、Windows 非法字符或保留名称；也不能以空白开头、结尾或以句点结尾。

重命名只修改下载名称，原下载链接、文件内容及 SHA-256 校验值保持不变。平台与架构仍只允许在草稿中调整；改名不会让隐藏或下架的文件公开。
...
接口测试使用隔离临时目录，覆盖登录、权限边界、项目、版本、上传、文件改名、下载、范围请求、下架和重启持久化。`scripts/check-ui.mjs` 使用 Playwright 验证真实浏览器全流程，包括更新日志解析及后台改名交互，测试数据不写入正式站点。
