...
- 安装包拖放选择、上传前改名、上传后原位改名、平台和架构设置、草稿附件移除、正式发布与下架。
- 可使用键盘的自定义下拉框、项目支持平台标签、精简的密钥登录表单。
...
登录右侧只保留欢迎信息、密钥输入、显隐按钮、验证按钮和错误提示；返回发布中心的入口仍可直接访问。项目、渠道、平台、架构等选择项使用自定义下拉框，支持方向键、Enter / 空格确认、Escape 关闭。项目支持平台采用可勾选标签，支持 Tab 聚焦与空格切换。
...
3. 选择安装包，在待上传列表中核对或修改完整文件名、平台与架构；点击上传按钮后，页面先保存草稿，再上传真实文件。
...
已发布或已下架版本的安装包内容不能覆盖，也不能增删附件或修改附件的平台、架构。软件内容有变化时应创建新版本；安装包名称可单独修改，具体方式见下文。
...
上传前，选择文件只会加入待上传列表，可直接在文件所在行编辑包含扩展名的完整名称；核对平台与架构后再点击上传。修改的是本次上传采用的名称，不会修改电脑上的原文件。

上传后，在版本编辑页点击文件旁的「重命名」，即可在原位置编辑，再保存或取消。草稿、已发布和已下架附件均可改名；保存成功后，列表、发布预览和浏览器下载文件名同步更新，不会丢失版本表单中尚未保存的编辑。后台「文件管理」仍使用重命名弹窗。
...
接口测试使用隔离临时目录，覆盖登录、权限边界、项目、版本、上传、文件改名、下载、范围请求、下架和重启持久化。`pnpm test:ui` 依次运行 `scripts/check-ui.mjs` 与 `scripts/check-carousel.mjs`，使用 Playwright 验证真实浏览器发布流程、原位改名、自定义下拉及轮播；测试数据不写入正式站点。浏览器脚本默认使用本地 Codex 运行时的 Playwright 和 Chrome，可通过 `DESIGN_NODE_MODULES`、`DESIGN_BROWSER` 指定安装路径。
