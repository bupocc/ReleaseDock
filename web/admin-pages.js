...
function releaseTable(items, full = false, projects = []) {
  return `<div class="table-scroll"><table class="data-table release-table"><thead><tr><th scope="col">项目 / 版本</th>${full ? '<th scope="col">发布渠道</th>' : ''}<th scope="col">状态</th><th scope="col">更新时间</th><th scope="col" class="align-right">下载次数</th><th scope="col" class="align-right">操作</th></tr></thead><tbody>${items.map((release) => {
    ...
    return `<tr data-row data-state="${e(release.status)}" data-search="${e(`${project.name} ${release.version} ${release.title}`)}"><td><div class="table-project">${projectMark(project, 'small')}<div><strong>${e(project.name)}</strong><span class="mono">${e(versionLabel(release.version))}</span></div>${release.isLatest && release.status === 'published' ? '<span class="table-latest">最新</span>' : ''}</div></td>${full ? `<td><span class="tag ${release.channel === 'prerelease' ? 'tag-orange' : 'tag-green'}">${release.channel === 'prerelease' ? '预发布版' : '稳定版'}</span></td>` : ''}<td>${statusBadge(release.status)}</td><td class="table-date mono">${formatDate(release.updatedAt || release.publishedAt)}</td><td class="align-right mono table-number">${release.downloadCount == null ? '—' : formatNumber(release.downloadCount)}</td><td class="align-right"><div class="release-row-actions"><a class="table-action" href="${url('publish', { id: release.id })}">${release.status === 'draft' ? '继续编辑' : '版本详情'} ${icon(release.status === 'draft' ? 'edit' : 'arrow', 13)}</a>${publicLink ? `<a class="table-action" href="${url('project', { slug: project.slug, release: release.id })}" aria-label="查看 ${e(project.name)} ${e(versionLabel(release.version))} 的公开页面">${icon('external', 14)}${full ? '公开页' : ''}</a>` : ''}${full && release.status === 'published' ? `<button class="table-action withdraw-action" data-withdraw="${e(release.id)}" data-release-name="${e(`${project.name} ${versionLabel(release.version)}`)}">下架</button>` : ''}</div></td></tr>`;
...
function settingsPage(data) {
  return `${pageHeading('站点设置', '调整发布中心的名称与公开介绍。')}<form class="panel site-settings-form" id="site-settings-form"><div class="panel-heading"><h2>基本信息</h2><span class="tag tag-outline">站点配置</span></div><div class="settings-fields"><label><span class="form-label">站点名称 <span class="required">*</span></span><input name="name" required maxlength="80" value="${e(data.site.name)}"></label><label><span class="form-label">站点简介</span><textarea rows="3" name="description" maxlength="1000" placeholder="简要介绍你的软件发布中心">${e(data.site.description)}</textarea><span class="field-hint">显示在公开首页的介绍区域。</span></label><label><span class="form-label">站点公告</span><textarea rows="3" name="announcement" maxlength="2000" placeholder="可选，在公开首页显示的公告">${e(data.site.announcement)}</textarea><span class="field-hint">留空即可隐藏公告。</span></label><div class="settings-notice">${icon('shield', 18)}<p>管理员密钥通过服务器配置管理。此页不显示或更改密钥。</p></div><p class="inline-error" id="settings-error" role="alert"></p><p class="inline-success" id="settings-success" role="status"></p><button class="btn btn-primary" type="submit">${icon('check', 15)}保存设置</button></div></form>`;
}
