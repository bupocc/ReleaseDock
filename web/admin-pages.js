...
function fileRow(file) {
  return `<tr data-row data-asset-row="${e(file.id)}" data-state="${e(file.status)}" data-search="${e(`${file.filename} ${file.projectName} ${file.version}`)}"><td><div class="file-name-cell">${icon('file', 18)}<div class="file-name-details"><span class="mono" data-asset-filename title="${e(file.filename)}">${e(file.filename)}</span>${assetRenameButton(file, 'file-rename-action')}</div></div></td><td>${e(file.platform || '通用')}<span class="table-subline mono">${e(file.arch || '—')}</span></td><td class="mono">${formatBytes(file.size)}</td><td>${e(file.projectName)}<span class="table-subline mono">${e(displayVersion(file.version))}</span></td><td>${statusBadge(file.status)}</td><td class="align-right"><div class="release-row-actions">${file.sha256 ? `<button class="table-action" type="button" data-file-checksum="${e(file.sha256)}" aria-label="复制 ${e(file.filename)} 的校验值">SHA-256 ${icon('copy', 12)}</button>` : ''}<a class="table-action" href="${url('publish', { id: file.releaseId })}">所属版本 ${icon('arrow', 12)}</a></div></td></tr>`;
}
