...
// 公共界面图标使用本地矢量图形，项目标识可使用管理员上传的图片。
const paths = {
  ...
  android: '<path d="m7 5-2-3m12 3 2-3M5 10a7 7 0 0 1 14 0H5Zm0 3v5a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-5M2 12v6m20-6v6M8 20v2m8-2v2"/><path d="M9 7h.01M15 7h.01"/>',
  phone: '<rect x="6" y="2" width="12" height="20" rx="3"/><path d="M10 5h4m-3 14h2"/>',
  harmony: '<circle cx="12" cy="10" r="7"/><path d="M5 21h14"/>',
...
export function projectMark(name, size = '') {
  const project = typeof name === 'object' && name !== null ? name : { slug: name };
  const sizeClass = ['small', 'large'].includes(size) ? size : '';
  if (typeof project.icon === 'string' && /^\/api\/icons\/[a-zA-Z0-9_-]+$/.test(project.icon)) {
    return `<span class="project-mark uploaded ${sizeClass}"><img src="${escapeHtml(project.icon)}" alt="" loading="lazy" decoding="async"></span>`;
  }
  name = String(project.slug || '');
  const shapes = {
    box: '<path d="m24 7 17 9-17 9-17-9 17-9Zm-17 9v18l17 9 17-9V16M24 25v18M15 12l17 9"/>',
  ...
  const mark = Object.hasOwn(shapes, name) ? name : 'box';
  return `<span class="project-mark ${mark} ${sizeClass}"><svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${shapes[mark]}</svg></span>`;
}
