...
import { escapeHtml as e, formatBytes, formatDate, formatNumber, versionLabel, safeWebsite, renderNotes, copyText } from './api.js';
import { header, footer, platformIcons, platformIcon, url, toast } from './app.js';
import { renderPasskeyLogin, bindPasskeyLogin } from './passkeys.js';
import { platformLabel } from './platforms.js';
import { renderProjectCarousel, bindProjectCarousel } from './project-carousel.js';
...
function siteAnnouncement(site) {
  return site.announcement ? `<aside class="site-announcement">${icon('info', 16)}<p>${e(site.announcement)}</p></aside>` : '';
}
...
function downloadRows(assets) {
  return assets.map((file) => `<div class="download-row"><span class="os-mark">${icon(platformIcon(file.platform), 22)}</span><div class="download-file"><div><strong>${e(platformLabel(file.platform))}</strong>${file.arch ? `<span class="tag">${e(file.arch)}</span>` : ''}</div><p class="mono">${e(file.filename)}</p></div><span class="file-size mono">${formatBytes(file.size)}</span>${file.sha256 ? `<button class="checksum" data-checksum="${e(file.sha256)}" data-filename="${e(file.filename)}" aria-label="复制 ${e(file.filename)} 的 SHA-256 校验值">SHA-256 ${icon('copy', 11)}</button>` : ''}<a class="btn btn-primary btn-sm" href="/api/downloads/${encodeURIComponent(file.id)}" download aria-label="下载 ${e(file.filename)}">${icon('download', 14)}下载</a></div>`).join('');
}
...
export function bindPublic(page, params, data) {
  ...
  if (page === 'login') bindPasskeyLogin(params, data);
  if (page === 'project') {
